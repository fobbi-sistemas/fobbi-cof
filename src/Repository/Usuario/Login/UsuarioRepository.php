...
namespace Src\Repository\Usuario\Login;
...
class UsuarioRepository extends GenericRepository
{
    ...
    {
        try {
            if ($dados['login'] != null && ! empty($dados['login']) && $dados['senha'] != null
                && ! empty($dados['senha'])) {
                ...
                foreach ($result as $usuario) {
                    if (strtolower($usuario['login']) == strtolower($dados['login'])
                            && password_verify($dados['senha'], $usuario['senha'])) {
                        
                        $_SESSION['idUsuarioAdminSite'] = $usuario['id'];
    ...
    {
        try {
            $sql = "UPDATE usuario SET dataUltimoAcesso = :dataUltimoAcesso WHERE id = :id;";
